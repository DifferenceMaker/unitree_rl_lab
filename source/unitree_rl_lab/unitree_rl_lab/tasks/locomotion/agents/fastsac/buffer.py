# Ported from Amazon FAR's holosoma (Apache-2.0) fast_sac_utils.py.
# Port notes (aspired/fastsac, 2026-08-25): TensorDict replaced by plain
# nested dicts (used purely as key-value containers); the distributed branch
# of EmpiricalNormalization dropped (single-GPU port). Math VERBATIM.
...
class SimpleReplayBuffer(nn.Module):
    """Circular per-env replay buffer with n-step returns + asymmetric obs."""

    def __init__(self, n_env, buffer_size, n_obs, n_act, n_critic_obs,
                 n_steps=1, gamma=0.99, device=None):
        super().__init__()
        self.n_env = n_env
        ...
        self.critic_observations = torch.zeros((n_env, buffer_size, n_critic_obs), device=device)
        self.next_critic_observations = torch.zeros((n_env, buffer_size, n_critic_obs), device=device)
        self.ptr = 0

    def extend(self, td: dict):
        ptr = self.ptr % self.buffer_size
        self.observations[:, ptr] = td["observations"]
        self.actions[:, ptr] = td["actions"]
        self.rewards[:, ptr] = td["next"]["rewards"]
        self.dones[:, ptr] = td["next"]["dones"]
        self.truncations[:, ptr] = td["next"]["truncations"]
        self.next_observations[:, ptr] = td["next"]["observations"]
        self.critic_observations[:, ptr] = td["critic_observations"]
        self.next_critic_observations[:, ptr] = td["next"]["critic_observations"]
        self.ptr += 1
    ...
    def sample(self, batch_size: int) -> dict:
        if self.n_steps == 1:
            ...
            # n-step > 1 path — ported verbatim from holosoma; the fastsac
            # default (num_steps=1) never enters here.
