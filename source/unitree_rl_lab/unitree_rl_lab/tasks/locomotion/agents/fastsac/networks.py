...
# Paper: "Learning Sim-to-Real Humanoid Locomotion in 15 Minutes" (FastSAC).
# Port (aspired/fastsac, 2026-08-25): CNN variants dropped; math VERBATIM.
...
class Actor(nn.Module):
    ...
    def get_actions_and_log_probs(self, obs: torch.Tensor):
    ...
    @torch.no_grad()
    def explore(self, obs: torch.Tensor, dones=None, deterministic: bool = False):
        _, mean, log_std = self(obs)
        if deterministic:
            if self.use_tanh:
...
class DistributionalQNetwork(nn.Module):
    def __init__(self, n_obs, n_act, num_atoms, v_min, v_max, hidden_dim,
                 use_layer_norm=True, device=None):
        super().__init__()
        self.net = nn.Sequential(
    ...
    def projection(self, obs, actions, rewards, bootstrap, discount, q_support, device):
        ...
        lower_indices = torch.clamp((lower + offset).view(-1), 0, proj_dist.numel() - 1)
        upper_indices = torch.clamp((upper + offset).view(-1), 0, proj_dist.numel() - 1)
...
class Critic(nn.Module):
    def __init__(self, n_obs, n_act, num_atoms, v_min, v_max, hidden_dim,
                 use_layer_norm=True, num_q_networks=2, device=None):
        super().__init__()
        if num_q_networks < 1:
            raise ValueError("num_q_networks must be at least 1")
        self.qnets = nn.ModuleList([
            DistributionalQNetwork(n_obs=n_obs, n_act=n_act, num_atoms=num_atoms,
                                   v_min=v_min, v_max=v_max, hidden_dim=hidden_dim,
