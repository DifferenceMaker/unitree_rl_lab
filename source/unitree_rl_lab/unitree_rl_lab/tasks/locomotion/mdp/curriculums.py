...
def lin_vel_cmd_levels(
    ...
    # gate_frac (lm4b, 2026-08-14): the episode sum is normalized by MAX episode
    # length, so the gate couples tracking quality WITH survival — a perfect
    # tracker dying at 40% of the episode reads 0.4*weight and can never clear
    # 0.8. Default 0.8 preserves upstream (unitree h1/g1/go2) behavior; the lm4b
    # trunk runs 0.5 (fires at genuinely-decent quality once survival is high).
    ...
    if env.common_step_counter % env.max_episode_length == 0:
        if reward > reward_term.weight * gate_frac:
