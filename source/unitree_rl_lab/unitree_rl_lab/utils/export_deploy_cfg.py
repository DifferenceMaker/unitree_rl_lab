...
def export_deploy_cfg(env: ManagerBasedRLEnv, log_dir):
    ...
    for action_name, action_term in action_terms:
        ...
        if action_name in ["JointPositionAction", "JointVelocityAction"]:
            # RelativeJointPositionActionCfg has use_zero_offset instead of
            # use_default_offset — its offset is always zero.
        ...
        # Deploy runner must know the target transform: absolute
        # (default_offset + scale*a) vs relative (measured_q + scale*a,
        # LieDown line / HoST β bound).
        cfg["actions"][action_name]["relative"] = "Relative" in type(action_term).__name__
