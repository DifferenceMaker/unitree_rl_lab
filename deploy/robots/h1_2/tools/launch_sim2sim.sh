...
#   launch_sim2sim.sh --sidecar   + balance_metrics sidecar      (3rd tab, tv env)
#
# Builds the sim and the controller first (incremental — no-op if up to date).
# All three run on `lo`, DDS domain 0 (matches simulate/config.yaml).
#
...
exec bash
EOF
chmod +x /tmp/sim2sim_mujoco.sh /tmp/sim2sim_ctrl.sh /tmp/sim2sim_sidecar.sh
...
echo ">>> launching Konsole ($([[ $SIDECAR -eq 1 ]] && echo '3 tabs: sim + controller + sidecar' || echo '2 tabs: sim + controller')) ..."
...
$([[ $SIDECAR -eq 1 ]] && echo "    TAB 3 sidecar    — metrics print + publish to the sim HUD (bottom-left)")
